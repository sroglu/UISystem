...
namespace EnhancedUI.EnhancedScroller
{
    ...
    // CUSTOM MODIFICATIONS STARTS // Primary cell view changed delegate
    /// <summary>
    /// This delegate is called when the primary cell view changes - assuming each cell fills the entire viewable area of the scroller
    /// </summary>
    public delegate void PrimaryCellViewChanged(EnhancedScrollerCellView cellView);

    public delegate void PadderVisibilityChanged(bool isTopPadder, bool isVisible);
    // CUSTOM MODIFICATIONS ENDS //
    ...
    public class EnhancedScroller : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        // CUSTOM MODIFICATIONS STARTS
        // Primary cell view changed delegate
        public PrimaryCellViewChanged primaryCellViewChanged;
        // Padder visibility changed delegate
        public PadderVisibilityChanged padderVisibilityChanged;
        // CUSTOM MODIFICATIONS ENDS //
        ...
        public void Snap()
        {
            ...
            // CUSTOM MODIFICATIONS STARTS // Direction-based snap watch offset.
            var directionalSnapWatchOffset = snapWatchOffset;
            var velocityDirection = Mathf.Sign(LinearVelocity);
            if (scrollDirection == ScrollDirectionEnum.Horizontal && velocityDirection < 0)
            {
                directionalSnapWatchOffset = 1f - snapWatchOffset;
            }
            else if (scrollDirection == ScrollDirectionEnum.Vertical && velocityDirection > 0)
            {
                directionalSnapWatchOffset = 1f - snapWatchOffset;
            }
            // CUSTOM MODIFICATIONS ENDS // Direction-based snap watch offset.
            // stop the scroller
            LinearVelocity = 0;
            ...
            */ // NEW CODE:
            var snapPosition = ScrollPosition + ScrollRectSize * Mathf.Clamp01(directionalSnapWatchOffset);
            // CUSTOM MODIFICATIONS ENDS // Direction-based snap watch offset.
        ...
        private void _SetPadders()
        {
            ...
            // CUSTOM MODIFICATIONS STARTS
            bool isFirstPadderIsActive = _firstPadder.gameObject.activeSelf;
            bool isLastPadderIsActive = _lastPadder.gameObject.activeSelf;
            // CUSTOM MODIFICATIONS ENDS
            ...
            // CUSTOM MODIFICATIONS STARTS
            if (_firstPadder.gameObject.activeSelf != isFirstPadderIsActive)
            {
                padderVisibilityChanged?.Invoke(true, _firstPadder.gameObject.activeSelf);
            }
            if (_lastPadder.gameObject.activeSelf != isLastPadderIsActive)
            {
                padderVisibilityChanged?.Invoke(false, _lastPadder.gameObject.activeSelf);
            }
            // CUSTOM MODIFICATIONS ENDS
        }
        ...
        IEnumerator TweenPosition(TweenType tweenType, float time, float start, float end, Action tweenComplete, bool forceCalculateRange)
        {
            ...
            else
            {
                ...
                // CUSTOM MODIFICATIONS STARTS // Primary cell view changed delegate
                if (primaryCellViewChanged != null)
                {
                    primaryCellViewChanged.Invoke(GetPrimaryCellView());
                }
                // CUSTOM MODIFICATIONS ENDS //
        ...
        // CUSTOM MODIFICATIONS STARTS // Primary cell view changed delegate
        private EnhancedScrollerCellView GetPrimaryCellView()
        {
            if (_activeCellViews.Count == 0) return null;

            float scrollerCenter;
            if (scrollDirection == ScrollDirectionEnum.Vertical)
            {
                scrollerCenter = _scrollPosition + (_scrollRectTransform.rect.height / 2f);
            }
            else
            {
                scrollerCenter = _scrollPosition + (_scrollRectTransform.rect.width / 2f);
            }

            EnhancedScrollerCellView primaryCellView = null;
            float closestDistance = float.MaxValue;

            for (var i = 0; i < _activeCellViews.Count; i++)
            {
                var cellView = _activeCellViews[i];
                float cellCenter = _cellViewOffsetArray[cellView.cellIndex] - (_cellViewSizeArray[cellView.cellIndex] / 2f);
                float distance = Mathf.Abs(cellCenter - scrollerCenter);
                if (distance < closestDistance)
                {
                    closestDistance = distance;
                    primaryCellView = cellView;
                }
            }

            return primaryCellView;
        } 
        // CUSTOM MODIFICATIONS ENDS //
